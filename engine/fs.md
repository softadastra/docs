...
Windows uses another.

Polling may be needed as a fallback.
...
observe the filesystem and produce structured state or events
Main concepts
...
unless the API intentionally accepts strings for convenience.
...
path
metadata
optional content hash or extra state
...
Same input state should produce the same snapshot.

Same before and after snapshots should produce the same diff.
...
The fs module can produce file events.

The wal module can record events.
...
The store contains current application state.

The filesystem module observes disk state.
...
fs should not know about discovery.
...
Metadata can describe a node.

FS can observe files.
...
Do not use fs when you only need key-value store operations.

For key-value state, use store.
...
Some files or directories may not be readable.

The scanner should either:

return an error
or
record partial scan behavior explicitly if supported

Do not silently ignore permission errors unless the API explicitly documents that behavior.

Watcher failure

A watcher can fail when:

path is invalid
path does not exist
permission denied
native backend cannot start
...
Deleted files need careful handling.

If a file is deleted, the current filesystem state no longer contains it.

The event should still tell the caller which path was deleted.
...
size changed
hash changed, if supported

If only timestamp and size are used, some changes may be missed.

Hash support can improve accuracy but costs more.
...
Deleted files appear in the before snapshot but not the after snapshot.
...
Updated files appear in both snapshots but with changed metadata.
...
This is acceptable for a deterministic snapshot-based model.

A future optimized watcher can add rename detection if needed.
...
Use it when learning how to read the full filesystem state.

Snapshot example summary

snapshot.cpp teaches:

build snapshot
iterate entries
inspect file states

Use it when learning the snapshot model.

Diff example summary
...
Use it when learning change detection.
...
print event type and path
keep process alive

Use it when learning real-time observation.
...
No changes is normal.

A diff can return an empty list.
...
Native watchers can overflow or behave differently across platforms.

Snapshot diff remains the safer deterministic model.

Mixing CLI output into fs

fs should return data.

CLI should format that data.
